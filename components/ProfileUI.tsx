    ...
    return (
        <div className="min-h-screen pt-24 pb-12 px-6 lg:px-8 bg-dark-100">
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                {/* --- Profile Header --- */}
                <div 
                    className="relative p-8 rounded-3xl overflow-hidden border border-white/5 shadow-2xl"
                    style={{ background: "linear-gradient(145deg, rgba(30,32,44,0.9), rgba(15,17,26,0.95))" }}
                >
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-8 pt-8 border-t border-white/5">
                        {stats.map((stat) => (
                            <div key={stat.label} className="text-center md:text-left">
                                <p className="text-xs font-medium text-gray-500 uppercase tracking-widest mb-1">{stat.label}</p>
                            ...
                            <div className="grid gap-4">
                                {interviews.length > 0 ? interviews.map((item: any) => (
                                    <Link key={item.id} href={`/interview/${item.id}`}>
                                        <div className="group flex items-center justify-between p-5 rounded-2xl bg-white/5 border border-white/5 hover:border-indigo-500/30 transition-all hover:bg-white/[0.07]">
                                            <div className="space-y-1">
                                                <h3 className="text-white font-semibold">{item.role}</h3>
                                                <p className="text-gray-500 text-xs">{dayjs(item.createdAt).format("MMM D, YYYY • h:mm A")}</p>
                                            </div>
                        ...
                        {activeTab === "quizzes" && (
                            <div className="grid gap-4">
                                {quizzes.length > 0 ? quizzes.map((item: any) => (
                                    <div key={item.id} className="flex items-center justify-between p-5 rounded-2xl bg-white/5 border border-white/5">
                                        <div className="space-y-1">
                                            <h3 className="text-white font-semibold">{item.topic || "AI Quiz"}</h3>
                                            <p className="text-gray-500 text-xs">{dayjs(item.createdAt).format("MMM D, YYYY")}</p>
                                        </div>
                                        <div className="flex flex-col items-end gap-1">
                                            <span className="text-lg font-bold text-white">{item.score}%</span>
                                            <span className="text-[10px] text-gray-500 uppercase tracking-widest font-medium">Score</span>
                                        </div>
                                    </div>
                        ...
                        {activeTab === "prep" && (
                            <div className="grid gap-4">
                                {prepSessions.length > 0 ? prepSessions.map((item: any) => (
                                    <div key={item.id} className="flex items-center justify-between p-5 rounded-2xl bg-white/5 border border-white/5">
                                        <div className="space-y-1">
                                            <h3 className="text-white font-semibold">{item.role || "Job Prep"}</h3>
                                            <p className="text-gray-500 text-xs">{dayjs(item.createdAt).format("MMM D, YYYY")}</p>
                                        </div>
...
function EmptyState({ message, link }: { message: string, link: string }) {
    return (
        <div className="flex flex-col items-center justify-center py-12 px-6 text-center border-2 border-dashed border-white/5 rounded-3xl bg-white/[0.02]">
            <p className="text-gray-400 mb-6">{message}</p>
            <Link href={link}>
                <button className="px-6 py-2.5 bg-white/10 hover:bg-white hover:text-black transition-all rounded-xl font-bold text-sm">
                    Start Now
                </button>
            </Link>
        </div>
